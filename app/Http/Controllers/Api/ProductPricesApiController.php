...
class ProductPricesApiController extends Controller
{
...
//        DB::connection()->enableQueryLog();
        ...
        Log::error('====== get prodocut prices ===== country_code ' . $country_code);

        $productPrices = ProductPrice::with('product')
            ->when(($country_code && $country_code != self::ALL_COUNTRIES), function ($query) use ($country_code) {
...
//        Log::error('====== get prodocut prices ===== ' . json_encode($queries));
...
//        Log::error('====== orders get ====== ' . json_encode($orders));
    ...
    {
        $productPrices = ProductPrice::orderBy('country_name', 'asc')->get();
